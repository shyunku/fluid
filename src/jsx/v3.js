...
(function () {
  ...
  function parse(input) {
    ...
    function collectText() {
      let txt = "";
      while (["TEXT", "IDENT", "NUMBER", "KEYWORD"].includes(peek().type)) {
        txt += consume().value;
      }
    ...
    function parseNode() {
      ...
      if (["TEXT", "IDENT", "NUMBER", "KEYWORD"].includes(t.type)) {
        // IDENT/NUMBER/KEYWORD 토큰이 태그 외부에 등장 → 일반 텍스트로 처리
        return collectText();
      }
  ...
  function generate(node) {
    switch (node.type) {
      ...
      case "Text": {
        // 공백만 있는 경우는 출력하지 않음 → buttononClick 오류 및 렌더링 최적화
