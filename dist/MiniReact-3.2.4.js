...
var MiniReact = (() => {
  ...
  var Fiber = class _Fiber {
    ...
    clone() {
      ...
      newFiber.hooks = this.hooks.map((hook) => ({
        ...hook,
        queue: hook.queue ? hook.queue.slice() : []
      }));
      return newFiber;
    }
  };
  ...
  function useReducer(reducer, initialState) {
    const wipFiber = Cache.wipFiber;
    const oldHook = wipFiber.alternate ? wipFiber.alternate.memoizedState : null;
    if (Cache.wipHook) {
      Cache.wipHook = Cache.wipHook.next;
    } else {
      Cache.wipHook = wipFiber.memoizedState;
    }
    if (!Cache.wipHook) {
      Cache.wipHook = {
      ...
      if (wipFiber.memoizedState === null) {
        wipFiber.memoizedState = Cache.wipHook;
      } else {
      }
    }
    const queue = Cache.wipHook.queue;
    if (queue.pending) {
      let firstUpdate = queue.pending.next;
      let newState = Cache.wipHook.memoizedState;
      let update = firstUpdate;
      while (update) {
        newState = reducer(newState, update.action);
        update = update.next;
      }
      Cache.wipHook.memoizedState = newState;
      queue.pending = null;
    }
    if (!queue.dispatch) {
    ...
    return [Cache.wipHook.memoizedState, queue.dispatch];
  }
  ...
  function createContext(defaultValue) {
    console.log("CreateContext", defaultValue);
    const context = {
    ...
    switch (fiber.tag) {
      ...
      case NodeTagType.COMPONENT: {
        try {
          ...
          const hasPendingUpdates = fiber.updateQueue !== null && fiber.updateQueue.pending !== null;
